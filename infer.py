...
model_id = "/workspace/diffused-video-trainer/models/dangerzone/MaSDV"
...
init_image = Image.open("/workspace/diffused-video-trainer/img.png").convert("RGB").resize((512, 512))
...
for p in prompts:
    for i in range(100):
        images = pipe(p, image=init_image, mask_image=mask_image, num_inference_steps=100, guidance_scale=12.0, frames_length=24).images
        counter_j = 0
