...
class TransformerPseudo3DModel(ModelMixin, ConfigMixin):
    """
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, return_dict: bool = True):
        ...
        # 2. Blocks
        for block in self.transformer_blocks:
            hidden_states = block(hidden_states, context=encoder_hidden_states, timestep=timestep, frames_length=f, height=height, weight=weight)
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.attn_temporal = LocalTemporalAttention(
            query_dim=dim,
            heads=num_attention_heads,
            dim_head=attention_head_dim,
            dropout=dropout,
        ...
        if self.use_ada_layer_norm:
            ...
            self.norm_temporal = AdaLayerNorm(dim, num_embeds_ada_norm)
        else:
    ...
    def forward(self, hidden_states, context=None, timestep=None, frames_length=None, height=None, weight=None):
        # 1. Self-Attention
        norm_hidden_states = (
        ...
        hidden_states = self.attn2(norm_hidden_states, context=context, frames_length=frames_length) + hidden_states

        # append temporal attention
        if frames_length is not None:
            hidden_states = rearrange(hidden_states, '(b f) (h w) c -> (b h w) f c', f=frames_length, h=height, w=weight)
            norm_hidden_states = (
            ...
            hidden_states = rearrange(hidden_states, '(b h w) f c -> (b f) (h w) c', f=frames_length, h=height, w=weight)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, hidden_states, context=None, mask=None, frames_length=None):
        ...
        if not has_cond and self.d_head <= 64:
...
class LocalTemporalAttention(nn.Module):
    ...
    def forward(self, hidden_states, context=None, mask=None, frames_length=None):
        ...
        """
        if self.local_attn.attention_mask is None or self.local_attn.attention_mask.shape[1] != query.shape[1]:
            new_mask = self.local_attn._get_local_mask(query.shape)
            #new_mask = new_mask.to_bool()
            new_mask = new_mask.to_dense()
            new_mask[:,0] = True
            #new_mask = AttentionMask.from_bool(new_mask)
            self.local_attn.attention_mask = new_mask.to(query.device)
        """
        hidden_states = self.local_attn(query, key, value)
        hidden_states = hidden_states.to(query.dtype)
        #hidden_states = self.normal_attention(query, key, value)
